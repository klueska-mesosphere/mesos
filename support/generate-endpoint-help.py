...
def generalize_endpoint_id(id):
  """Generalizes the id of the form e.g. slave(1) to slave(id)."""
  return re.sub("\([0-9]+\)", "(id)", id)


def normalize_endpoint_id(id):
  """Normalizes the id of the form e.g. slave(1) to slave."""
...
def write_markdown(path, output):
  ...
  # there are any.
  output = MARKDOWN_HEADER + output.rstrip()
