...
public:
  ...
  struct SubprocessInfo
  {
    ...
    public:
      ...
      Option<int_fd> fd() const { return *fd_->get(); }
    ...
    private:
      ...
      // `close()` since only one reference will ever exist to it.
      class FDWrapper
      {
      public:
     ...
     * See `process::Subprocess::IO`.
     */
    IO in = SubprocessInfo::IO::FD(STDIN_FILENO, false);

    /**
     * How to redirect the stdout of the executable.
     * See `process::Subprocess::IO`.
     */
    IO out = SubprocessInfo::IO::FD(STDOUT_FILENO, false);

    /**
     * Similar to `out`, except this describes how to redirect stderr.
     */
    IO err = SubprocessInfo::IO::FD(STDERR_FILENO, false);
  };
