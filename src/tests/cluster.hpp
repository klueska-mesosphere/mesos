...
public:
  ...
  static process::Owned<Master> start(
...
private:
  ...
  // Dependencies that were created by the factory method.
  ...
  process::Owned<MasterDetector> masterDetector;
...
public:
  ...
  static process::Owned<Slave> start(
  ...
  // Stops this slave by dispatching a shutdown call to the
  // underlying slave process.  If this is called, this helper object
  // will not perform cleanup when this object is destructed.
  void shutdown();

  // Stops this slave by terminating the underlying slave process.
  // If this is called, this helper object will not perform cleanup when
...
private:
  ...
  // Containerizer that is not managed by this pointer.
  // It is either owned outside of this object or by `ownedContainerizer`.
