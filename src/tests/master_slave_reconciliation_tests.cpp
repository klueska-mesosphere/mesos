...
TEST_F(MasterSlaveReconciliationTest, SlaveReregisterTerminatedExecutor)
{
  ...
  Future<StatusUpdateAcknowledgementMessage> statusUpdateAcknowledgementMessage
    = FUTURE_PROTOBUF(
        StatusUpdateAcknowledgementMessage(), master->pid, slave->pid);
...
TEST_F(MasterSlaveReconciliationTest, ReconcileRace)
{
  ...
  DROP_PROTOBUFS(ReregisterSlaveMessage(), slave->pid, master->pid);

  Future<ReregisterSlaveMessage> reregisterSlaveMessage =
    DROP_PROTOBUF(ReregisterSlaveMessage(), slave->pid, master->pid);
  ...
  FUTURE_PROTOBUF(ReregisterSlaveMessage(), slave->pid, master->pid);

  process::post(slave->pid, master->pid, reregisterSlaveMessage.get());
...
TEST_F(MasterSlaveReconciliationTest, SlaveReregisterPendingTask)
{
  ...
  Future<Nothing> _runTask = DROP_DISPATCH(slave->pid, &Slave::_runTask);
