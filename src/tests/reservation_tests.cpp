...
TEST_F(ReservationTest, ResourcesCheckpointing)
{
  ...
  Future<CheckpointResourcesMessage> checkpointResources =
    FUTURE_PROTOBUF(CheckpointResourcesMessage(), _, slave->pid);
...
TEST_F(ReservationTest, MasterFailover)
{
  ...
  Owned<cluster::Master> master1 = StartMaster(masterFlags);

  slave::Flags slaveFlags = CreateSlaveFlags();
  slaveFlags.resources = "cpus:8;mem:2048";

  StandaloneMasterDetector detector(master1->pid);
  ...
  Owned<cluster::Master> master2 = StartMaster(masterFlags);
...
TEST_F(ReservationTest, NoAuthentication)
{
  ...
  MockScheduler sched;
  MesosSchedulerDriver driver(&sched, frameworkInfo, master->pid);
