...
TEST_F(SlaveTest, ShutdownUnregisteredExecutor)
{
  ...
  Owned<cluster::Slave> slave = StartSlave(
      detector.get(), containerizer.get());
...
TEST_F(SlaveTest, CommandExecutorWithOverride)
{
  ...
  environment["MESOS_SLAVE_PID"] = stringify(slave->pid);
...
TEST_F(SlaveTest, IgnoreNonLeaderStatusUpdateAcknowledgement)
{
  ...
  Future<Message> frameworkRegisteredMessage = FUTURE_MESSAGE(
      Eq(FrameworkRegisteredMessage().GetTypeName()), master->pid, _);
...
TEST_F(SlaveTest, StateEndpoint)
{
  ...
  Future<Response> response = process::http::get(slave->pid, "state");
  ...
  EXPECT_EQ(stringify(slave->pid), state.values["pid"]);
  ...
  response = http::get(slave->pid, "state");
...
TEST_F(SlaveTest, DISABLED_TerminatingSlaveDoesNotReregister)
{
  ...
  Future<SlaveReregisteredMessage> slaveReregisteredMessage =
    DROP_PROTOBUF(SlaveReregisteredMessage(), master->pid, slave->pid);
  ...
  EXPECT_NO_FUTURE_PROTOBUFS(
      ReregisterSlaveMessage(), slave->pid, master->pid);

  // Drop the ShutdownExecutorMessage, so that the slave will
  // stay in TERMINATING for a while.
  DROP_PROTOBUFS(ShutdownExecutorMessage(), slave->pid, _);
  ...
  // to avoid blocking.
  post(master->pid, slave->pid, ShutdownMessage());
...
TEST_F(SlaveTest, RateLimitSlaveShutdown)
{
  ...
  master::Flags masterFlags = CreateMasterFlags();
  Owned<cluster::Master> master = StartMaster(slaveRemovalLimiter, masterFlags);
...
TEST_F(SlaveTest, DiscoveryInfoAndPorts)
{
  ...
  Future<Response> response = process::http::get(slave->pid, "state.json");
...
TEST_F(SlaveTest, HTTPScheduler)
{
  ...
  Future<RunTaskMessage> runTaskMessage =
    DROP_PROTOBUF(RunTaskMessage(), master->pid, slave->pid);
  ...
  process::post(master->pid, slave->pid, spoofed);
...
TEST_F(SlaveTest, HTTPSchedulerLiveUpgrade)
{
  ...
  process::post(master->pid, slave->pid, updateFrameworkMessage);
...
TEST_F(SlaveTest, HTTPSchedulerSlaveRestart)
{
  ...
  process::post(master->pid, slave->pid, spoofedRunTaskMessage);
  ...
  process::post(master->pid, slave->pid, spoofedUpdateFrameworkMessage);
  ...
  process::post(executorPid, slave->pid, executorToFrameworkMessage);
