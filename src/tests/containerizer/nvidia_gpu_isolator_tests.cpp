...
// This test verifies that we can enable the Nvidia GPU isolator and
// launch tasks with restricted access to GPUs while running inside a
// docker container. We first launch a task with 1 GPU and verify that
// a call to `nvidia-smi` both succeeds and reports exactly 1 GPU
// available. We then launch a task with access to 0 GPUs and verify
// that a call to `nvidia-smi` fails.
