...
TEST_F(RateLimitingTest, NoRateLimiting)
{
  ...
  // FrameworkRegisteredMessage back after processing it.
  {
    ...
    process::post(schedulerPid, master->pid, subscribeCall.get());
...
TEST_F(RateLimitingTest, DifferentPrincipalFrameworks)
{
  ...
  MesosSchedulerDriver* driver1 =
    new MesosSchedulerDriver(&sched1, frameworkInfo1, master->pid);
  ...
  MesosSchedulerDriver driver2(&sched2, frameworkInfo2, master->pid);
  ...
  // head of the queue.
  {
    ...
    process::post(sched1Pid, master->pid, subscribeCall1.get());
    process::post(sched2Pid, master->pid, subscribeCall2.get());
...
TEST_F(RateLimitingTest, SchedulerFailover)
{
  ...
  process::post(schedulerPid, master->pid, subscribeCall2.get());
