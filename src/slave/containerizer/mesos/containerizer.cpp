    ...
    const Option<state::SlaveState>& state)
{
  ...
  if (containerIds.isError()) {
    ...
    foreach (const ContainerID& containerId, containerIds.get()) {
      ...
      Result<pid_t> pid = containerizer::paths::getContainerPid(
    ...
    bool checkpoint)
{
  ...
  return logger->prepare(
    ...
    // the container.
    //
    // NOTE: This is different than the checkpoint information
    // requested by the agent via the `checkpoint` parameter.
