...
{
  // Ten is more than enough digits
  // to hold the encoded exit status.
  char statusString[10];
  int total = ::sprintf(statusString, "%d", status);

  if (total < 0) {
    cerr << "Failed to write the container status"
         << " '" << statusString << "' to"
